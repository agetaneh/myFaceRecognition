...
def load_model():
    #use VGG16 to load lower layers of vgg16 network and declare it as base_model
    #use 'imagenet' for weights, include_top=False, (IMG_H, IMG_W, NUM_CHANNELS) for input_shape
    base_model = vgg16.VGG16(include_top=False, weights='imagenet', input_tensor=None, input_shape=(IMG_H, IMG_W, NUM_CHANNELS),pooling=None, classes=NUM_CLASSES)

    print('Model weights loaded.')
    base_out = base_model.output
    #add a flatten layer, a dense layer with 256 units, a dropout layer with 0.5 rate,
    C1 = Flatten()(base_model)
    C2 = Dense(256) (C1)
    C3 = Dropout(0.5) (C2)

    # another dense layer for output. The final layer should have the same number of units as classes
    C4 = Dense(NUM_CLASSES, activation='softmax')(C3)

    model = Model(inputs=base_model.input, outputs=predictions)
    print ('Build model')
    model.summary()

    # compile the model, use SGD(lr=1e-4,momentum=0.9) for optimizer, 'categorical_crossentropy' for loss,
